#!/usr/bin/env python3
"""Smoke-test the kamae-scala plugin package without external dependencies."""
...
SKILL_ROOT = ROOT / "skills" / "kamae-scala"
...
def check_json(errors: list[str]) -> None:
    for path in [
        ...
        try:
            ...
            fail(errors, f"{path.relative_to(ROOT)}: invalid JSON at {exc.lineno}:{exc.colno}: {exc.msg}")
...
def check_manifest_skill_paths(errors: list[str]) -> None:
    ...
    for plugin in data.get("plugins", []):
        ...
        if plugin.get("name") == "kamae-scala" and not skills:
            fail(errors, f"{marketplace.relative_to(ROOT)}: kamae-scala entry missing skills")
        for entry in skills:
            path = (ROOT / entry).resolve()
...
def check_skill_frontmatter(errors: list[str]) -> None:
    for path in sorted((ROOT / "skills").glob("*/SKILL.md")):
        ...
        if data.get("name") and data["name"] != expected:
            fail(errors, f"{rel}: frontmatter name {data['name']!r} does not match directory {expected!r}")
...
LIBRARY_GUIDE_RE = re.compile(r"references/library-guides/[A-Za-z0-9_-]+\.md")


def check_library_guides(errors: list[str]) -> None:
    for path in sorted(
        [ROOT / "rules" / "defaults" / "library-detection.md", ROOT / "skills" / "kamae-scala" / "SKILL.md"]
    ):
        text = path.read_text(encoding="utf-8")
        for guide in sorted(set(LIBRARY_GUIDE_RE.findall(text))):
            guide_path = SKILL_ROOT / guide
            if not guide_path.is_file():
                fail(errors, f"{path.relative_to(ROOT)}: referenced library guide does not exist: {guide}")
...
def check_python_scripts(errors: list[str]) -> None:
    script_dirs = [
        ROOT / "scripts",
        SKILL_ROOT / "scripts",
        ROOT / "skills" / "kamae-scala-review" / "scripts",
    ]
...
def main() -> int:
    ...
    print(
        "Checked JSON manifests, skill frontmatter, Markdown links, manifest skill paths, library-guide references, and Python scripts."
    )
    return 0
